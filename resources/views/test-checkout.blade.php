...
<head>
    ...
    <style>
        ...
        .field input { width: 100%; padding: 8px 12px; box-sizing: border-box; }
        button { background: #3395ff; color: #fff; border: none; padding: 10px 20px; cursor: pointer; font-size: 1rem; border-radius: 6px; }
        button:hover { background: #2878dd; }
        .result { margin-top: 24px; padding: 16px; background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; }
        .result h3 { margin-top: 0; }
        .result pre { background: #fff; padding: 12px; overflow-x: auto; font-size: 12px; }
        .note { font-size: 0.875rem; color: #666; margin-top: 8px; }
...
<body>
    ...
    @if(!$razorpay_key)
        <div class="result error">
            <strong>Razorpay not configured.</strong> Ensure an active payment gateway with code "Razorpay" exists and has <code>key_id</code> in credentials.
        </div>
    @else
        <div class="field">
        ...
        <div class="field">
            ...
            <input type="number" id="amount" placeholder="99" value="99" min="1">
        </div>
        <div class="field">
        ...
        <div id="payment_result" class="result" style="display: none;">
            ...
            <p class="note">POST /api/v1/payment/verify with header <code>X-Android-Id: your_android_id</code> and body (JSON) with transaction_id, gateway_order_id, gateway_payment_id, gateway_signature.</p>
        </div>
    @endif
